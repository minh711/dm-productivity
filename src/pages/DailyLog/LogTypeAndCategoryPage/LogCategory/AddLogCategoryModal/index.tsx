...
interface AddLogCategoryModalProps {
  open: boolean;
  onClose: () => void;
  onAdd: (logType: LogCategory) => void;
}
...
const AddLogCategoryModal: React.FC<AddLogCategoryModalProps> = ({
  open,
  onClose,
  onAdd,
}) => {
  ...
  const handleOk = () => {
    form.validateFields().then((values) => {
      const newLogCategory: LogCategory = {
        id: uuidv4(),
        name: values.name,
...
export default AddLogCategoryModal;
